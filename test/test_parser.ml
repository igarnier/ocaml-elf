...
let string_of_e_class cls =
  match cls with ELFCLASS32 -> "ELF32" | ELFCLASS64 -> "ELF64"
...
let print_elf e =
  ...
  let () =
    printf "  Class:                         %s\n" (string_of_e_class e.e_class)
  in
  let () =
    printf "  Data:                          %s\n" (string_of_e_data e.e_data)
  in
  let () = printf "  Ident Version:                 %d\n" e.e_version in
  let () =
    printf "  OS/ABI:                        %s\n" (string_of_e_osabi e.e_osabi)
  in
  let () = printf "  ABI Version:                   %d\n" e.e_abiver in
  let () =
    printf "  Type:                          %s\n" (string_of_e_type e.e_type)
  in
  let () =
    printf
      "  Machine:                       %s\n"
      (string_of_e_machine e.e_machine)
  in
  ...
  let print_shdr n s =
    let () =
      printf
        "[%2d] %-22s %-10s %016Lx %016Lx %2Ld %-5s %2ld %3ld %2Ld\n"
        n
        s.sh_name
        (string_of_sh_type s.sh_type)
        s.sh_addr
        s.sh_size
        s.sh_entsize
        (String.concat "" (List.map string_of_sh_flag s.sh_flags))
        s.sh_link
        s.sh_info
        s.sh_addralign
    in
  ...
  let () =
    printf
      " Type             VirtAddr         PhysAddr         MemSiz           \
       Flg Align\n"
  in
  let print_phdr p =
    printf
      " %-16s %016Lx %016Lx %016Lx %-3s %016Lx\n"
      (string_of_p_type p.p_type)
      p.p_vaddr
      p.p_paddr
      p.p_memsz
      (String.concat "" (List.map string_of_p_flag p.p_flags))
      p.p_align
  in
